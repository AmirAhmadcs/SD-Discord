...
export default function Composer({ disabled, disabledReason, mediaDisabled, mediaDisabledReason, onSend, placeholder, onOpenSchedule }) {
  ...
  async function handleFilePick(e) {
    ...
    try {
      ...
      setPendingFile({ kind: kindFromMime(file.type), name: file.name, size: file.size, dataUrl });
    } catch {
  ...
  async function handleSubmit(e) {
    ...
    if (!text.trim()) return;
    const result = onSend({ kind: 'text', text });
    if (!result.ok) {
      setError(result.error);
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ borderTop: '1px solid var(--border-soft)', padding: 14 }}>
      ...
      {pendingFile && (
        ...
        >
          {pendingFile.kind === 'image' ? (
            <img src={pendingFile.dataUrl} alt="" style={{ width: 40, height: 40, borderRadius: 8, objectFit: 'cover' }} />
          ) : (
            <FileText size={20} color="var(--accent-text)" />
          )}
          <span style={{ fontSize: 13, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {pendingFile.name} <span style={{ color: 'var(--text-muted)' }}>({formatBytes(pendingFile.size)})</span>
          </span>
          <button
            type="button"
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8 }}>
        ...
        <button
          ...
          title={mediaDisabled ? mediaDisabledReason : 'Attach a photo, video, audio or file'}
          ...
          style={{
            ...
            border: '1px solid var(--border-mid)',
            background: 'var(--bg-input)',
            color: mediaDisabled ? 'var(--text-muted)' : 'var(--text-secondary)',
            display: 'flex',
            alignItems: 'center',
        ...
        <textarea
          ...
          placeholder={pendingFile ? 'Add a caption (optional)' : placeholder || 'Write a message...'}
