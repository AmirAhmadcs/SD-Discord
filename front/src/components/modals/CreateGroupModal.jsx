...
export default function CreateGroupModal({ users, onClose, onCreate }) {
  ...
  function submit(e) {
    e?.preventDefault();
    const result = onCreate({ name, memberIds: selected });
    if (!result.ok) {
  ...
  return (
    ...
    >
      <form onSubmit={submit}>
        ...
        <div style={{ maxHeight: 260, overflowY: 'auto', border: '1px solid var(--border-mid)', borderRadius: 10 }}>
          {users.length === 0 && (
            <p style={{ padding: 14, color: 'var(--text-muted)', fontSize: 13, margin: 0 }}>No other users yet.</p>
          )}
          {users.map((u) => {
