...
export default function MembersModal({
  ...
  function handleAdd(userId) {
    const result = onAddMember(userId);
    if (!result.ok) setError(result.error);
    else setError('');
  }

  function handleRoleChange(userId, roleId) {
    const result = onAssignRole(userId, roleId);
    if (!result.ok) setError(result.error);
    else setError('');
  }

  return (
    <Modal title={`${kind === 'channel' ? 'Channel' : 'Group'} members`} onClose={onClose} width={460}>
      ...
      {canManageMembers && nonMembers.length > 0 && (
        <div style={{ marginTop: 14 }}>
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, padding: '7px 12px' }}
          >
            <UserPlus size={15} />
            Add member
          </button>
          {showAdd && (
            <div style={{ marginTop: 10, maxHeight: 180, overflowY: 'auto', border: '1px solid var(--border-mid)', borderRadius: 10 }}>
              {nonMembers.map((u) => (
                <button
                  key={u.id}
                ...
                >
                  ...
                  <span style={{ fontSize: 11.5, color: 'var(--text-muted)', marginLeft: 4 }}>@{u.username}</span>
                </button>
              ))}
            </div>
          )}
